...
lower= np.array([100,50,50],np.uint8) # Array (H,S,V) for the lower threshold bound of the HSV image
...
def mouseEvent(event, x, y, flags, param):
	...
	check_lower = np.uint16(originalH - error[0])
        ...
        while(check_lower < 0):
            check_lower = 255 + check_lower #Hue wrap
            lower[0] = np.uint8(check_lower)
        while(check_upper > 255):
            check_upper = check_upper - 255 #Hue wrap
            upper[0] = np.uint8(check_upper)
...
def get_image(CompressedImage):
    ...
    # All the images used to find the ball are made global so we can display them durring debugging (lazy and globaling them).
    global imgBGR
    global imgHSV
    global imgBLUR
    global mask
